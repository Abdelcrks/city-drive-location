            ...
            <div class="space-y-4">
                <div   class="group relative overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950 shadow-[0_20px_80px_-40px_rgba(0,0,0,0.8)]" >
                    <div class="relative aspect-16/10">
                        <img src={images[activeIndex]}  alt={car.model} class="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" />
                ...
                {#if images.length > 1}
                    ...
                    <div class="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                        {#each images as img, index}
                            ...
                            >
                                <img src={img} alt="" class="h-16 w-24 object-cover cursor-pointer" />
                            </button>
                        {/each}
                    </div>
                {/if}
